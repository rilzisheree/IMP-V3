...
export const data = new SlashCommandBuilder()
  .setName('lt')
  .setDescription('Grant a user the Lore Team role across all affiliated servers (Owner only)')
  .addUserOption(opt =>
    opt.setName('user')
      .setDescription('User to grant Lore Team in all servers')
      .setRequired(true)
  );
...
export async function execute(interaction) {
  ...
  if (Object.keys(GUILD_ROLE_MAP).length === 0) {
    return interaction.editReply({
      content: '⚠️ No servers configured in `GUILD_ROLE_MAP`. Open `lt.js` and add your guild ID + role ID pairs.',
    });
  }
  ...
  for (const guild of interaction.client.guilds.cache.values()) {
    ...
    try {
      ...
      await member.roles.add(roleId, `Lore Team granted by ${interaction.user.tag} via /lt`);
  ...
  const embed = new EmbedBuilder()
    .setColor(0xF5C400)
    .setTitle('Lore Team Role Granted')
