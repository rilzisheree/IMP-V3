...
const COMMANDS = ['purge', 'say', 'serverlist', 'globalban', 'unglobalban', 'globalbanlist', 'setlogchannel', 'dm', 'allowuser', 'wave', 'unwave', 'appeal'];
...
export async function execute(interaction) {
  ...
  if (sub === 'add') {
    ...
    if (existing) {
      return interaction.editReply({
        embeds: [new EmbedBuilder()
          .setColor(0xF5C400)
          .setDescription(`**${target.tag}** already has permission to use \`/${command}\`.`)]
      });
    }
    ...
    if (!deleted) {
      return interaction.editReply({
        content: `**${target.tag}** didn't have permission for \`/${command}\`.`
      });
    }
    ...
    if (records.length === 0) {
      return interaction.editReply({
        embeds: [new EmbedBuilder()
          .setColor(0xF5C400)
          .setDescription('No custom permissions are set in this server.')]
      });
    }
